...
// InEnum 检查值是否在枚举列表中
func InEnum(enums ...string) string {
	var enumStr string
	for _, enum := range enums {
		enumStr += enum + ","
	}
	return "enum=" + strings.TrimRight(enumStr, ",")
}
	...
	for i := 0; i < num; i++ {
		...
		if len(roleMap[tagVal.Name]) > 0 {
			for _, v := range roleMap[tagVal.Name] {
				...
				case strings.Split(v, "=")[0] == "enum":
					if !inEnum(val.String(), strings.Split(v, "=")[1]) {
						fmt.Println(val.String(), strings.Split(v, "=")[1])
						return errors.New(tagVal.Name + "格式校验不通过")
					}
...
func inEnum(value interface{}, VerifyStr string) bool {
	str, ok := value.(string)
	if !ok {
		return false
	}
	enums := strings.Split(VerifyStr, ",")
	for _, enum := range enums {
		if str == enum {
			return true
		}
